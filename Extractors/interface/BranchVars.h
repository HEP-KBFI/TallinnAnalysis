#ifndef TallinnAnalysis_Extractors_BranchVars_h
#define TallinnAnalysis_Extractors_BranchVars_h
...
class BranchVarBase
{
 ...
 protected:
  ...
  static std::map<std::string, const BranchVarBase *> instances_; // key = branchName
};

template <typename T>
class BranchVar : public BranchVarBase
{
 public:
  ...
  getValue() const
  {
    const BranchVar<T> * gInstance = dynamic_cast<const BranchVar<T> *>(instances_[branchName_]);
    assert(gInstance);
...
#endif // TallinnAnalysis_Extractors_BranchVars_h
